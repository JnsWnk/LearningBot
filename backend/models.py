...
def load_all_models(emb=True, tiny=True, flan=True):
    ...
    if (emb):
        try:
            print(
                f"CORE: Loading embedding model: {config.EMBEDDING_MODEL_NAME}...")
            device = 'cuda' if torch.cuda.is_available() else 'cpu'
            models["emb"] = SentenceTransformer(
    ...
    if (tiny):
        try:
            print(
                f"CORE: Loading TinyLlama: {config.BASE_LLM_NAME} + adapter {config.ADAPTER_NAME}...")
            compute_dtype = torch.bfloat16 if torch.cuda.is_available(
            ) and torch.cuda.is_bf16_supported() else torch.float16

            tokenizer_mini = AutoTokenizer.from_pretrained(
            ...
            print(
                f"CORE: Loading TinyLlama base with 4-bit quantization (Compute: {compute_dtype})...")
            ...
            llm_mini = PeftModel.from_pretrained(
                base_model_mini, config.ADAPTER_NAME)
            llm_mini.eval()
    ...
    if (flan):
        try:
            ...
            nlu_model = AutoModelForSeq2SeqLM.from_pretrained(
                NLU_MODEL_ID).to(nlu_device)
            print(
                f"CORE: Successfully loaded NLU model class: {type(nlu_model)}")
    ...
    if load_successful:
        print(
            f"CORE: Finished loading all required ML models in {end_time - start_time:.2f} seconds.")
    else:
        print(
            f"CORE: WARNING - Some models failed to load. Check errors above. Time taken: {end_time - start_time:.2f} seconds.")
    return models
