...
def interpret_intent(user_input: str, nlu_model, nlu_tokenizer, use_gpt4: bool = False) -> dict:
    if use_gpt4:
        intent_list = ["GET_INFORMATION",
                       "MANAGE_KNOWLEDGE", "REQUEST_REVIEW", "OTHER"]
...
def interpret_intent_flan(user_input: str, nlu_model, nlu_tokenizer) -> dict:
    ...
    try:
        inputs = nlu_tokenizer(prompt, return_tensors="pt",
                               max_length=128, truncation=True).to(nlu_model.device)
        ...
        result_text = nlu_tokenizer.decode(
        ...
        if ':' in result_text and '"' in result_text:
            ...
            try:
                ...
                valid_intents = ["GET_INFORMATION",
                                 "MANAGE_KNOWLEDGE", "REQUEST_REVIEW", "OTHER"]
                if intent not in valid_intents:
                    print(
                        f"  CHATBOT NLU Warning: Parsed intent '{intent}' not in valid list. Defaulting to OTHER.")
                    intent = "OTHER"
                ...
                print(
                    f"  CHATBOT NLU Error: JSONDecodeError after adding braces: {json_err}")
                print(
                    f"  String that failed parsing: '{json_string_to_parse}'")
                return {"intent": "OTHER", "topic": None}
        else:
            print(
                "  CHATBOT NLU Warning: Output doesn't look like key-value pairs. Defaulting to OTHER.")
            return {"intent": "OTHER", "topic": None}
...
def retrieve_context(query_text: str, embedding_model, index_name: str, k: int = 5) -> str:
    ...
    try:
        ...
        pipeline = [
            ...
            {'$project': {'_id': 0, 'text_chunk': 1,
                          'score': {'$meta': 'vectorSearchScore'}}}
        ]
        results = list(db.docs_collection.aggregate(pipeline))
        context = "\n---\n".join(
            [f"Context: {res['text_chunk']} (Score: {res['score']:.4f})" for res in results])
    ...
    try:
        if use_gpt4:
            if True:
                ...
                client = genai.Client(
                    api_key="AIzaSyA0qlJtfONXi5Fyoj3fRhNPFoFA1XyVQvo")
...
def process_chat_message(
    ...
    if not user_data:
        ...
        print(
            f"Found user data for ID {user_id}: {user_data.get('knowledge_profile', {})}")

    intent_data = interpret_intent(
        user_input, models["nlu"], models["tok_nlu"], use_gpt4)
    ...
    if intent == "GET_INFORMATION":
        if use_gpt4:
            prompt = prompts.create_get_information_prompt(
                user_input, context, chat_history)
        else:
            prompt = prompts.create_get_information_prompt_tinyl(
                user_input, context, chat_history)
        bot_answer = generate_response(
            prompt, models["mini"], models["tok_mini"], use_gpt4)
        ...
        if not topic:
            ...
            prompt = prompts.create_manage_knowledge_prompt(
                topic, current_level, context, user_data, chat_history)
            bot_answer = generate_response(
                prompt, models["mini"], models["tok_mini"], True)
            is_quiz = True
        ...
        current_level = get_topic_level(user_data, topic)
        prompt = prompts.create_request_review_prompt(
            topic, current_level, context, user_data, chat_history)
        bot_answer = generate_response(
            prompt, models["mini"], models["tok_mini"], True)
        is_quiz = True
        ...
        prompt = prompts.create_other_prompt(
            user_input, chat_history, user_data)
        bot_answer = generate_response(
            prompt, models["mini"], models["tok_mini"], True)
...
def process_quiz(
    ...
    try:
        ...
        evaluation_prompt = prompts.create_quiz_evaluation_prompt(
            question, answer, topic)

        # Get evaluation from GPT-4
        evaluation_response = generate_response(
            evaluation_prompt, None, None, use_gpt4=True)
        ...
        try:
            ...
            if "```json" in evaluation_response:
                ...
                json_str = evaluation_response.split(
        ...
        if user_data and standardized_topic in user_data.get("knowledge_profile", {}):
            current_level = user_data["knowledge_profile"][standardized_topic].get(
                "level", 1)
